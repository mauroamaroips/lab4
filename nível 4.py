...
# que permite criar objetos que podem ser compartilhados entre processos.
...
# chaves com valores None até ao número máximo de chaves informado em qtd_valores.

# [p.join() for p in lista_de_processos]: aguarda o fim de cada processo da lista_de_processos usando a função join().
# Existe uma iteração sobre cada processo na lista_de_processos
# sendo chamado join() para aguardar a finalização de cada um deles.
...
# quantidade de termos a serem calculados (são inicializados 4 processos).

# Por exemplo, se a quantidade de termos a serem calculados for muito grande, o número de processos criados pode
# tornar-se excessivo, levando a uma sobrecarga do sistema e a um aumento no tempo total de processamento.
...
# Por exemplo, uma implementação utilizando recursividade pode levar a ganhos significativos de desempenho (como verificado
# no nível 3 deste laboratório).